	...
	public void execute() {
		try {
			...
			executeUpdate() : 
				...
				실행후 적용된 행의 갯수가 int형으로 반환된다.
			executeQuery() : 
				select 쿼리문을 실행할때 사용하는 메서드로 레코드에 영향을 미치지않고, 단순히 조회만 하는경우 사용한다.
			...
			while(rs.next()) {
				...
				0000-00-00 00:00:00 형식이므로 적당한 크기로 자르고 싶다면 substring() 메서드를 사용하면 된다.
				 */
				String regidate2 = rs.getString("regidate");
				String regidate3 = rs.getString("regidate").substring(0, 13);
				/*
				오라클의 변환함수를 통해 출력하고자 하는 형태로 포매팅한후 문자형식으로 출력한다.
