...
-클래스는 반드시 상속관계에서 설계도와 같은 역할을 한다.
-인스턴스 생성은 할수 없지만 그외 참조변수, 상속 등의 모든 기능을 사용할 수 있다.
...
아래 오버라이딩한 메서드는 삭제해도 에러가 발생하지 않는다.
 */
...
class AbsClassChild2 extends AbsClass2 {
	/*
	오버라이딩은 부모쪽의 메서드를 가리고,
	새로운 기능을 재정의 하는것이므로 해당 클래스는 추상메서드를 포함하지 않는 상태가된다.
	따라서 인스턴스 생성이 가능해진다.
