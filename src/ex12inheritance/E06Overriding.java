...
/*
오버라이딩(Overriding) : 
	클래스가 상속관계에 있을때 하위클래스에서 상위클래스의 멤버메서드와 완전히 동일한 모양으로 메서드를 재정의 하는것을 말한다.
	오버로딩과 다른점은 매개변수의 갯수 및 타입, 반환타입 등이 완전힌 똑같은 형태로 정의하는 것이다.
	하위클래스에서 오버라이딩한 메서드는 상위클래스의 메서드를 가리게 되므로 항상 하위클래스에서 오버라이딩한 메서드가 호출된다.
 */
...
public class E06Overriding {
	...
	public static void main(String[] args) {
		...
		자식쪽에 정의되어있는 setBase()는 호출할 수 없게된다.
		 */
		speaker.setVolumn(30);
//		speaker.setBase(40); // 에러발생
		/*
		접근범위는 앞에서 설명한 바와 같이 부모까지로 제한되지만 오버라이딩된 메서드가 있다면 자식쪽의 메서드가 호출된다.
		 */
		speaker.showState();
