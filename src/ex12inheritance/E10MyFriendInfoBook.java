...
class HighFriend extends Friend {
	...
	/*
	오버라이딩1 : 고딩친구의 전체정보를 출력하기 위해 부모에서 정의한 메서드를 super를 통해 호출하고,
...
public class E10MyFriendInfoBook {
	...
	/*
	무한루프 조건으로 사용자가 원할때 종료할수 있는 구조를 만들어 준다.
	break문은 반복문을 탈출을 탈출시키는 기능이 있으므로 이와같은 무한루프에서 자주 사용한다.
...
class FriendInfoHandler {
	...
	public void addFriend(int choice) {
		...
		if(choice==1) {
			...
			/*
			참조변수를 인스턴스 배열에 추가한다.
			인덱스로 사용된 변수의 초기값이 0이므로, 0번 인덱스에 추가한뒤 1 증가시킨다.
			이를 위해 '후위증가' 하고있다.
			 */
		...
		/*
		더이상 실행할 문장이 없으므로 해당 메서드는 메모리에서 소멸되고, 호출한 지점인 main 메서드로 돌아간다.
		 */
	}
	...
	public void showSimpleData() {
		/*
		만약 부모클래스인 Friend에 showBasicInfo()메서드를 정의하지 않으면 자식클래스에서는 오버라이딩을 할 수 없으므로
		개별적인 멤버 메서드를 정의해야한다.
	...
	public void deletInfo() {
		...
		/*
		배열의 원소 중 삭제된 원소의 인덱스값을 저장하기 위한 변수로 삭제한 후 빈자리를 채워넣을때 사용한다.
		...
		for(int i=0; i<numOfFriends; i++) {
			...
			if(deleteName.compareTo(myFriends[i].name)==0) {
				...
				참조값이 null이라는것은 참조할 인스턴스가 없다는 의미이므로 삭제한것과 동일한 결과가 된다.
				 */
				myFriends[i] = null;
				// 삭제된 원소의 index를 저장한다.
				deleteIndex = i;
		...
		if(deleteIndex==-1) {
			...
			만약 이 부분이 처리되지 않는다면 차후 검색이나 삭제시 NullPointerExeption이 발생할 수 있다.
			 */
