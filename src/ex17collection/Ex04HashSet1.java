...
-하지만 개발자가 정의한 클래스라면 hashCode(), equals()메서드를 적절히 오버라이딩 해야만 중복을 제거할 수 있다.
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		// size()를 통해 저장된 객체수를 알수있다. 5개 출력됨
		...
		while(itr.hasNext()) {
			...
			/*
			HashSet<E>에 저장시 모두 Object형으로 자동 형변환되어 저장되므로
			우리가 직접 정의한 클래스에 대해서만 instanceof를 통해 타입을 확인한 후 다운캐스팅하여 출력한다.
			만약 다운캐스팅을 하지 않으면 자식멤서에 접근할 수 없으므로 getName()을 호출할 수 없다.
			 */
		 ...
		 */
		System.out.println(set.contains("JAVA") ? "varString1 있음" : "varString1 없음");
