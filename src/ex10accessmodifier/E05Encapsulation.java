...
/*
캡슐화(Encapsulation) : 
	여러가지 업무 로직을 하나로 묶는다는 의미와, 업무의 순서까지 고려한 형태의 로직을 구성한다는 의미를 가지고있다.
...
차후에는 별도의 분석 작업없이 해당 클래스의 doProcess() 메서드만 호출하면 업무는 차질없이 진행될것이다.
 */
class EncapsulLogic {
	...
	void doProcess() {
		memberRegist.doMemberRegist(); // 가입
		autoLogin.doAutoLogin(); // 자동로그인
		congratulationPoint.doCongratulationPoint(); // 가입축하포인트
		firstLoginEvent.doFirstLoginEvent(); // 첫로그인이벤트
	}
}
public class E05Encapsulation {
	...
	public static void main(String[] args) {
		...
		캡슐화 이전의 코드에서는 개발자가 순서를 지키지 않은 상태에서도 실행이 가능한 코드가 된다.
		즉 업무의 차질이 발생할 수 있다.
		 */
		MemberRegist memberRegist = new MemberRegist();
		FirstLoginEvent firstLoginEvent = new FirstLoginEvent();
		...
		// 아래메서드 호출을 통해 업무를 순서에 맞게 진행할 수 있다.
