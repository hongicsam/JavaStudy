...
//		one.privateMethod(); // 에러발생
		one.defaultMethod();
		/*
		private으로 선언된 멤버에 외부클래스에서 접근하려면 public으로 선언된 멤버메서드를 통해 간접적으로 접근해야한다.
...
//		System.out.println("one.privateVar = " + two.privateVar); // 에러발생
//		System.out.println("one.defaultVar = " + two.defaultVar); // 에러발생
