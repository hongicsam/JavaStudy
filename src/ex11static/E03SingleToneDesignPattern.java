...
싱글톤 디자인 패턴 : 
	...
	하나의 인스턴스 즉 하나의 메모리를 할당한 후 이를 프로그램 전체에서 공유해서 사용하고자할때 쓰는 패턴이다.
...
5. 이 방법을 통해 인스턴스를 생성하면 메모리에 딱 하나만 만들어ㅣ지게되어 메모리를 절약할 수 있다.
 */
class NoSingleTone {
	int instVar;
	/*
	생성자는 거의 대부분 public으로 선언한다.
	만약 public으로 선언하지 않으면
	클래스외부 혹은 다른 패키지에서는 호출할 수 없기때문에 new를 통해 인스턴스를 생성할 수 없기때문이다.
	 */
	public NoSingleTone() {}
}
// 싱글턴 패턴이 적용된 클래스 정의
class SingleTone {
	...
	프로그램이 시작될때 미리 메모리(Method)에 로드되어 사용할 준비를 마치게된다.
	 */
	private static SingleTone single = new SingleTone();
	
	/*
	생성자를 private으로 선언하면 클래스 외부에서는 호출할 수 없으므로 new를 통해 인스턴스를 생성할 수 없게된다.
	 */
	private SingleTone() {}
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		/*
		일반적인 방식의 인스턴스 생성이므로, new로 생성할때마다 새로운 참조값(주소값)을 할당받게된다.
		 */
