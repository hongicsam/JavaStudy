...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		일반적인 멤버변수에 접근하는 방법 : MtStatic클래스의 인스턴스를 생성한 후 참조변수를 통해 접근한다.
		 */
		...
		static(정적) 멤버변수 접근방법 : 클래스명을 통해 직접 접근한다.
			인스턴스를 생성하지 않고 즉시 직접 접근할 수 있다.
		 */
		MyStatic.staticVar = 200;
		System.out.println("MyStatic.StaticVar = " + MyStatic.staticVar);
		/*
		정적변수도 일반적인 멤버변수처름 참조변수를 통해 접근할 수 있지만, Java에서는 권장하지 않는다.
		(※ 참조변수를 통해 접근할거면, 궅이 정적변수를 사용할 이유가 없기 때문이다.)
		 */
		...
		System.out.println("ms1.intanceVar = " + ms1.instanceVar);
		System.out.println("ms2.intanceVar = " + ms2.instanceVar);
		...
		정적변수에 접근시에는 참조변수를 사용하지 말고, 클래스명을 사용하는것이 좋다.
		...
		참조변수를 통해서는 인스턴스형, 정적형 둘다 접근할 수 있다.
		 */
		myStatic.instanceMethod();
		myStatic.staticMethod(); // 권장사항 아님
		/*
		클래스명을 통한 접근은 정적메서드에서만 가능하다.
		인스턴스 메서드는 인스턴스 참조변수를 통해서만 접근할 수 있다.
		 */
